...
test("1 Standard Registration attendee = US$45", () => {
...
test("10 Standard Registration attendees = US$450", () => {
...
test("10 individual attendees remain US$450 through the registration window", () => {
...
test("20 individual attendees remain US$900 through the registration window", () => {
  const result = calculateSummitPrice(
...
test("Standard Corporate Registration Package for 10 attendees = US$450", () => {
  const result = calculateSummitPrice({
    attendeeCount: 10,
...
test("Standard Corporate Registration Package for 20 attendees = US$900", () => {
  const result = calculateSummitPrice({
    attendeeCount: 20,
...
for (const [instant, registrationOpen] of [
  ...
  test(`Individual Standard Registration closes after October 1 in Trinidad: ${instant}`, () => {
    const result = calculateSummitPrice(
      { attendeeCount: 1, registrationType: "individual" },
      new Date(instant),
    );
    assert.equal(result.ok, registrationOpen);
    assert.equal(result.ok && result.summary.rateValue, registrationOpen ? "standard" : false);
    assert.equal(result.ok && result.summary.total, registrationOpen ? 45 : false);
  });

  test(`Corporate Standard Registration closes after October 1 in Trinidad: ${instant}`, () => {
